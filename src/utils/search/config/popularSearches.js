/** Shown when the search box is focused with an empty query. */
export const POPULAR_SEARCHES = [
  {label: 'Creating Leads', to: '/docs/Leads-Module/creating-leads'},
  {label: 'Import Contacts', to: '/docs/Contacts-Module/creating-contacts'},
  {label: 'Workflow Automation', to: '/docs/Automation/Workflows/Overview'},
  {label: 'Assignment Rules', to: '/docs/Pulse-Settings/AssignmentRules'},
  {label: 'Knowledge Base', to: '/docs/Pulse-Settings/KnowledgeBase'},
  {label: 'Roles', to: '/docs/user-management/roles'},
  {label: 'Business Hours', to: '/docs/Pulse-Settings/SupportAvailability'},
  {label: 'Deal Pipelines', to: '/docs/Configure/pipeline'},
];
